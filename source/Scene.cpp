...
Scene::~Scene() {
  ...
  while (target) {
    tr = target->transform();
    Foreach(tr->hierarchy,
            [&stack](Transform *c) { stack.emplace_back(c->gameObject()); });
...
void Scene::PinScript(Behaviour *behav) {
  if (!_firstRunScripts)
    GC::gc_push(_firstRunScripts);
  if (std::find_if(begin(*this->_firstRunScripts), end(*this->_firstRunScripts),
                   [behav](Behaviour *ref) { return behav == ref; }) ==
      end(*_firstRunScripts)) {
    if (_realtimeScripts &&
        std::find_if(begin(*this->_realtimeScripts),
                     end(*this->_realtimeScripts), [behav](Behaviour *ref) {
                       return behav == ref;
                     }) != end(*_realtimeScripts))
      return;
...
void Scene::RenderScene(SDL_Renderer *renderer) {
  ...
  if (_realtimeScripts) {
    Foreach(*_realtimeScripts, [](Behaviour *n) { n->OnUpdate(); });
  }
