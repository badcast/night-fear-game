...
Scene::~Scene() {
    ...
    if (_firstRunScripts) {
        GC::gc_unload(_firstRunScripts);
    }
    if (_realtimeScripts) {
        GC::gc_unload(_realtimeScripts);
    }
    if (_destructions) {
        GC::gc_unload(_destructions);
    }
...
void Scene::PinScript(Behaviour *behav) {
    if (!_firstRunScripts) GC::gc_push_lvalue(_firstRunScripts);
...
void Scene::RenderScene(SDL_Renderer *renderer) {
    if (_firstRunScripts) {
        if (!_realtimeScripts) {
            GC::gc_push_lvalue(_realtimeScripts);
