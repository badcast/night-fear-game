...
void Initialize_Fonts(bool optimizeDeffects) {
  ...
  if ((pfont->surfHilight = GC::GetSurface("font-arealike-hi")) == nullptr)
    throw std::bad_alloc();
      ...
      for (x = 0; x < pfont->data[i].w / 2; ++x) {
        ...
        if (!(flagBreaker & 1)) {
          for (y = rect_point.h - 1; y >= 0; --y) {
            ...
            pixel = (SDL_Color *)pfont->surfNormal->pixels +
...
void Render_String(SDL_Renderer *renderer, Rect_t rect, const char *text,
                   int len, int pfontWidth, TextAlign textAlign, bool textWrap,
                   bool hilight) {
  ...
  if (!rect.w)
    rect.w = textWidth;
  if (!rect.h)
    rect.h = pfont->fontSize
                 .y; // todo: для мультий строк требуется вычислить h высоту
