...
std::uint8_t g_buttons;
std::uint8_t b_mainMenu;
std::uint8_t b_generateNav;
std::uint8_t b_clearNav;
std::uint8_t b_showGizmo;
std::uint8_t b_add, b_sub;
...
void GameScene::awake() {
  ...
  b_mainMenu =
      this->ui->Push_Button("Назад в главное меню", {25, 25}, g_buttons);
  b_generateNav =
      this->ui->Push_Button("Генерировать Nav Mesh", {300, 25}, g_buttons);
...
void GameScene::start() {
  ...
  GC::gc_alloc_texture_from(
      &texture, GC::gc_resource_sdl_surface("test", FolderKind::GFX));
  ...
  GC::gc_alloc_texture_from(
      &texture, GC::gc_resource_sdl_surface("floor", FolderKind::TEXTURES));
  GC::gc_alloc_sprite_with(&spr, texture);
  spriteRenderer->setSprite(spr);
  ...
  GC::gc_alloc_texture_from(
      &texture, GC::gc_resource_sdl_surface("target", FolderKind::GFX));
  GC::gc_alloc_sprite_with(&spr, texture);
