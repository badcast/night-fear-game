...
namespace RoninEngine {
   ...
   inline int skip(const char* c, int len = INT_MAX) {
      ...
      strncpy(skipping + sizeof syntax.trim_segments, syntax.array_segments,
              sizeof(syntax.array_segments));
      for (i = 0; c[i] && i <= len;) {
   ...
   inline bool isProperty(const char* c, int len) {
      for (size_t i = 0; i < len; ++i)
         if (!(toupper(c[i]) >= 'A' && toupper(c[i]) <= 'Z' ||
               (i > 0 && config::is_number(c[i])) || c[i] == '_'))
            return false;
      return len != 0;
   }
   ...
   int ObjectParser::avail(ObjectParser::_StructType& entry, const char* source,
                           int len, int levels) {
      int i, j;
      ...
      for (i = 0; source[i] && i < len;) {
                     ...
                     if (valueType != DataType::Unknown) {
                        ...
                        case DataType::Real: {
                           ...
                           if (valueType == DataType::Number) {
                              ref = new double(static_cast<double>(
                                                  *(std::int64_t*)memory));
                              deallocate_class((std::int64_t*)memory);
                           } else
                              ref = (double*)memory;

                           ((vector<double>*)curNode.value)
                                 ->emplace_back(*ref);
                           deallocate_class(ref);
                           break;
                        }
         ...
         entry.insert(
                  make_pair(j = stringToHash(curNode.propertyName.c_str()), curNode));
   ...
   void ObjectParser::Deserialize(const char* filename) {
      ...
      if ((buf = (char*)malloc(sz)) == NULL) throw std::bad_alloc();
