...
static int Get_FormatType(const char* c, void** mem, DataType& out) {
    ...
    if (*c == syntax.format_string) {
        ...
        if (mem) *mem = GC::gc_push<bool>(temp == sizeof(syntax.true_string) - 1);
...
ObjectNode::~ObjectNode() {
    if (!decrementMemory() && value) {
        if (isStruct()) {
            GC::gc_unload((ObjectParser::_StructType*)value);
        } else if (isArray()) {
            switch (valueFlag >> 2) {
                case DataType::Boolean:
                    GC::gc_unload(toBooleans());
                    break;
                case DataType::String:
                    GC::gc_unload(toStrings());
                    break;
                case DataType::Number:
                    GC::gc_unload(toNumbers());
                    break;
                case DataType::Real:
                    GC::gc_unload(toReals());
                    break;
            }
        } else if (isValue()) {
            if (isString()) {
                GC::gc_unload((std::string*)value);
            } else
                GC::gc_unload(value);
        }
    }
                    ...
                    if (source[i] == syntax.obstacle)
                        ...
                        if (valueType != DataType::Unknown) {
                            if (arrayType == DataType::Unknown) {
                                ...
                                switch (arrayType) {
                                    case DataType::String:
                                        curNode.setMemory((void*)GC::gc_push<std::vector<std::string>>());
                                        break;
                                    case DataType::Boolean:
                                        curNode.setMemory((void*)GC::gc_push<std::vector<bool>>());
                                        break;
                                    case DataType::Real:
                                        curNode.setMemory((void*)GC::gc_push<std::vector<double>>());
                                        break;
                                    case DataType::Number:
                                        curNode.setMemory((void*)GC::gc_push<std::vector<std::int64_t>>());
                                        break;
                                }
                            ...
                            switch (arrayType) {
                                ...
                                case DataType::Real: {
                                    ...
                                    if (valueType == DataType::Number) {
                                        ...
                                        GC::gc_unload((std::int64_t*)memory);
                                    } else
                                        ref = (double*)memory;
