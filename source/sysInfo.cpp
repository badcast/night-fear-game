...
__declspec(dllimport) int __stdcall K32GetProcessMemoryInfo(
    void *Process, PROCESS_MEMORY_COUNTERS_EX *ppsmemCounters,
    unsigned long cb);
}
#elif __unix__
...
unix_process_memory unix_process_info_from_proc() {
  unix_process_memory upm;
...
unix_process_memory unix_process_info() {
   unix_process_memory upm;
   struct sysinfo s;
...
const size_t get_cpuCount() { return 0; }
