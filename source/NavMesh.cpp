...
namespace RoninEngine::AIPathFinder
{
	...
	decltype(Neuron_Struct::CostType)& Neuron::cost() {
		return *(decltype(Neuron_Struct::CostType)*)((uint8_t*)_data + sizeof(Neuron_Struct::flagType) + sizeof(Neuron_Struct::h));
	}
	decltype(Neuron_Struct::h)& Neuron::heuristic() {
		return *(decltype(Neuron_Struct::h)*)((uint8_t*)_data + sizeof(Neuron_Struct::flagType));
	}

	const int Neuron::weight() {
		return x * x + y * y;
	}

	const uint32_t Neuron::total() {
		return cost() + heuristic();
	}

	const bool Neuron::empty() {
		return total() == 0;
	}
	...
	NavMesh::NavMesh(size_t Width, size_t Height)
	{
		...
		this->pmemory = malloc(widthSpace * heightSpace * (sizeof(Neuron_Struct::flagType) + sizeof(Neuron_Struct::CostType) + sizeof(Neuron_Struct::h)));
		Neuron* p;
		for (Width = 0; Width < widthSpace; ++Width)
		{
			for (Height = 0; Height < heightSpace; ++Height)
			{
				...
				p->x = Width;
				p->y = Height;
				p->_data = ((uint8_t*)pmemory) + (Width * heightSpace + Height) * (sizeof(Neuron_Struct::flagType) + sizeof(Neuron_Struct::CostType) + sizeof(Neuron_Struct::h));
#ifdef _DEBUG
				...
				p->p_cost = (decltype(Neuron_Struct::CostType)*)((uint8_t*)p->_data + sizeof(Neuron_Struct::flagType) + sizeof(Neuron_Struct::h));
#endif
	...
	void NavMesh::clear()
	{
		memset(pmemory, 0, widthSpace * heightSpace * (sizeof(Neuron_Struct::flagType) + sizeof(Neuron_Struct::CostType) + sizeof(Neuron_Struct::h)));
	}
	...
	Neuron* NavMesh::neuron(Disposition point) {
		return neuron(point.x, point.y);
	}
	...
	list<Vec2> NavMesh::findSpaces(const int& x, const int& y, int radius) {
		return {};
	}
	NavResult NavMesh::find(NavMethodRule method, Neuron* firstNeuron, Neuron* lastNeuron, list<Neuron*>* result, NavAlgorithm algorithm)
	{
		NavResult pinfo;
		pinfo.map = this;
	...
	int GetMatrixMethod(NavMethodRule method, int8_t** matrixH, int8_t** matrixV) {
		static int8_t PLUS_H_POINT[]
		{ -1, 1, 0, 0 };
		static int8_t PLUS_V_POINT[]
		{ 0, 0, -1, 1 };

		static int8_t M_SQUARE_H_POINT[]
		{ -1,0,1,-1,1,-1,0,1 };
		static int8_t M_SQUARE_V_POINT[]
		{ -1,-1,-1,0,0,1,1,1 };

		static int8_t M_CROSS_H_POINT[]
		{
			-1,  1,
			-1,  1
		};
		static int8_t M_CROSS_V_POINT[]
		{
			-1, -1,
			 1,  1
		};
	...
	void AlgorithmUtils::AvailPoints(NavMesh* map,
