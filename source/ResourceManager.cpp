...
namespace RoninEngine::Runtime
{
	...
	void ResourceManager::ResourcesInitialize()
	{
		constexpr int bufferSize =
			sizeof(*_assocMultiFiles) +
			sizeof(*_assocMultiLoadedImages) +
			sizeof(*_assocMultiCacheTextures) +
			sizeof(*_assocSingleFile) +
			sizeof(*_assocLoadedImages) +
			sizeof(*_assocCacheTextures) +
			sizeof(*_assocCacheCursors) +
			sizeof(*_cacheClasses) +
			sizeof(*_assocLoadedMusic) +
			sizeof(*_unusedTextures) +
			sizeof(*_usedSprites);
		...
		_assocCacheTextures = (decltype(_assocCacheTextures))(_assocLoadedImages + 1);
	...
	void ResourceManager::UnloadAll(bool immediate) {
		...
		for (auto iter = begin(*_assocLoadedImages); iter != end(*_assocLoadedImages); ++iter)
		{
	...
	_free_caches:
		...
		Foreach(*_cacheClasses, [](auto iter) {deallocate_class(iter); });
		Foreach(*_unusedTextures, [](Texture* iter) {deallocate_class(iter); });
		Foreach(*_usedSprites, [](Sprite* spr) { deallocate_class(spr); });
	...
	void* ResourceManager::GoMemoryCache(void* cacheToFree) {
		if (find_if(begin(*_cacheClasses), end(*_cacheClasses),
			[&cacheToFree](auto& x) {return x == cacheToFree; }) == end(*_cacheClasses))
			_cacheClasses->emplace_back(cacheToFree);
		return cacheToFree;
	}

	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	...
	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	list<Texture*>* ResourceManager::LoadTextures(const string& packName, bool autoUnload) {
	...
	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	SDL_Surface* ResourceManager::GetSurface(const string& surfaceName, FolderKind pathOn)
	{
	...
	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	Texture* ResourceManager::GetTexture(const string& resourceName, FolderKind pathOn, bool autoUnload) {
		SDL_Surface* surf;
		Texture* texture;
	...
	Texture* ResourceManager::GetTexture(const string& resourceName, bool autoUnload) {
	...
	Texture* ResourceManager::GetTexture(const int w, const int h, const ::SDL_PixelFormatEnum format) {
	...
	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	SDL_Cursor* ResourceManager::GetCursor(const string& resourceName, const Vector2i& hotspot, bool autoUnload) {
		return GetCursor(GetSurface(resourceName), hotspot);
	}

	//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на ResourceManager::Unload()
	SDL_Cursor* ResourceManager::GetCursor(SDL_Surface* texture, const Vector2i& hotspot) {
		SDL_Cursor* cur = NULL;
		...
		if (texture)
		{
			...
			else
			{
				...
				if (cur)
					_assocCacheCursors->emplace(make_pair(texture, cur));
			}
		}
		return cur;
	}
	...
	Sprite* ResourceManager::GetSprite(int w, int h) {
		Sprite* spr;
		allocate_variable(spr, reinterpret_cast<Texture*>(NULL), Rect_t(0, 0, w, h));
	...
	bool ResourceManager::UnloadTexture(Texture* const texturePtr) {
		...
		{
			SDL_Surface* sfPtr = GetSurface(string(texturePtr->raw_namePtr), FolderKind::TEXTURES);
			...
			if (!sfPtr)
				...
				auto iter = find_if(_unusedTextures->begin(), _unusedTextures->end(), [texturePtr](Texture* f) {
					return f == texturePtr;
				});
	...
	void ResourceManager::Unload(SDL_Surface* unload) {
		--unload->refcount;
	}
}
