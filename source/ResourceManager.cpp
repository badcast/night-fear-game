...
// any type
template <typename T> struct type2index { // undefined types
  static const std::uint8_t typeIndex = InvalidType;
};
template <> struct type2index<std::remove_pointer_t<void *>> { // Any pointer
  static const std::uint8_t typeIndex = 0;
};
template <> struct type2index<SDL_Texture> { // SDL Texture
  static const std::uint8_t typeIndex = 1;
};
template <> struct type2index<SDL_Surface> { // SDL Surface
  static const std::uint8_t typeIndex = 2;
};
template <> struct type2index<Texture> { // Ronin Texture
  static const std::uint8_t typeIndex = 32;
};
template <> struct type2index<Sprite> { // Ronin Sprite
...
template <typename T> constexpr T *_deinit(T *m) {
  m->~T();
  return m;
}
...
list<Texture *> *GC::LoadTextures(const string &packName, bool autoUnload) {
  ...
  if (iter == end(*_assocMultiCacheTextures)) {
    GC::gc_push(_textures);
    for (auto i = begin(*surfaces); i != end(*surfaces); ++i) {
      gc_alloc_texture_from(&p, SDL_CreateTextureFromSurface(
                                    RoninApplication::GetRenderer(), *i));
...
SDL_Surface *GC::GetSurface(const string &surfaceName) {
  return GetSurface(surfaceName, FolderKind::GFX);
}

//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на
// ResourceManager::Unload()
Texture *GC::GetTexture(const string &resourceName, FolderKind pathOn,
                        bool autoUnload) {
  SDL_Surface *surf;
  Texture *texture;
...
//Для автоматического уничтожения ресурса, обязательно его нужно скинуть на
// ResourceManager::Unload()
SDL_Cursor *GC::GetCursor(const string &resourceName, const point_t &hotspot,
                          bool autoUnload) {
...
// GC Resources   ---------------------------------------------------
int GC::gc_resource_sdl_surface(const string &surfaceName, FolderKind pathOn,
...
template <typename T> bool GC::gc_unload(T *pointer) {
  int id;
...
int GC::gc_alloc_sdl_texture(SDL_Texture **sdltexturePtr, const int &w,
  ...
  auto &&gc_ptr = reinterpret_cast<SDL_Texture *>(
      mem->memory = SDL_CreateTexture(RoninApplication::GetRenderer(), format,
                                      access, w, h));
...
int GC::gc_alloc_sdl_texture(SDL_Texture **sdltexturePtr, SDL_Surface *from) {
  ...
  id = gc_wrte_memblock<SDL_Texture>(&mem);
  mem->memory =
      SDL_CreateTextureFromSurface(RoninApplication::GetRenderer(), from);
...
int GC::gc_alloc_sprite_with(Sprite **spritePtr, Texture *texture,
                             const Rect_t &rect, const Vec2 &center) {
  int id;
  MemoryStick *ms;
...
template <typename T, typename X> X GC::gc_alloc_scene() {
  MemoryStick *m;
  int id;
...
template <typename T> bool GC::valid_type() {
