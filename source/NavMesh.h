...
public:
  ...
  NavResult find(NavMethodRule method, Neuron *firstNeuron, Neuron *lastNeuron,
                 list<Neuron *> *result, NavAlgorithm algorithm);
...
public:
  ...
  static auto GetMinCostPath(list<Neuron *> *paths)
      -> decltype(std::begin(*paths));

  /// Определяет, функцию пойска по направлениям. Таких как: left, up, right,
  static void AvailPoints(NavMesh *map, NavMethodRule method, Neuron *arrange,
                          Neuron *target, list<Neuron *> *pathTo,
