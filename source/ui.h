...
public:
  ...
  CI uid Push_Label(const string &text, const Rect_t &rect,
                     const int &fontWidth = 13, const uid &parent = NOPARENT);
  CI uid Push_Label(const string &text, const point_t &point,
                     const int &fontWidth = 13, const uid &parent = NOPARENT);
  CI uid Push_Button(const string &text, const Rect_t &rect,
                      const uid &parent = NOPARENT);
  CI uid Push_Button(const string &text, const point_t point,
                      const uid &parent = NOPARENT);
  CI uid Push_DisplayRandomizer(TextRandomizer_Format format,
                                 const point_t &point,
                                 const uid &parent = NOPARENT);
  CI uid Push_DisplayRandomizer(
      TextRandomizer_Format format = TextRandomizer_Format::All,
      const uid &parent = NOPARENT);
  CI uid Push_DisplayRanomizer_Text(const string &text, const point_t &point,
                                     const uid &parent = 0);
  CI uid Push_DisplayRanomizer_Number(const int &min, const int &max,
                                       TextAlign textAlign,
                                       const uid &parent = NOPARENT);
  CI uid Push_TextureStick(Texture *texture, const Rect_t &rect,
                            const uid &parent = NOPARENT);
  CI uid Push_TextureStick(Texture *texture, const point_t point,
                            const uid &parent = NOPARENT);
  CI uid Push_TextureAnimator(Timeline *timeline, const Rect_t &rect,
                               const uid &parent = NOPARENT);
  CI uid Push_TextureAnimator(Timeline *timeline, const point_t &point,
                               const uid &parent = NOPARENT);
  CI uid Push_TextureAnimator(const list<Texture *> &roads, float duration,
                               TimelineOptions option, const Rect_t &rect,
                               const uid &parent = NOPARENT);
  CI uid Push_TextureAnimator(const list<Texture *> &roads, float duration,
                               TimelineOptions option, const point_t &point,
                               const uid &parent = NOPARENT);
