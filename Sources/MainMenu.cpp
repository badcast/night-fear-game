...
namespace RoninEngine
{
	...
	void MainMenu::awake()
	{
		...
		{
            gui->Push_Label("Графика", { gameDisplay.w / 2 - 127, 300 }, fontWidth, group_setting_screen);
			ssb_backmenu = gui->Push_Button("< Назад", lastPoint, group_setting_screen);
		}
		//Author & About US
		group_author_screen = gui->Create_Group();
		{
            gui->Push_Label("Night Fear\n\nLight Mister (~^Y^~)\nИ ваш любимый котик {^_^}", { gameDisplay.w / 2 - 127, 300 }, fontWidth, group_author_screen);
...
#define e4(ID1,ID2, ID3, ID4)	ID1==id || ID2==id || ID3==id || ID4==id
		...
		{
            Application::LoadScene(GC::gc_alloc<GameScene>());
		}
		//setting menu events
