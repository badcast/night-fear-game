...
namespace RoninEngine::AIPathFinder
{
	...
	public:
		...
		__forceinline const bool locked();
		__forceinline void lock(const bool state);

		__forceinline decltype(Neuron_Struct::flagType)& flag();
		__forceinline decltype(Neuron_Struct::CostType)& cost();
		__forceinline decltype(Neuron_Struct::h)& heuristic();

		__forceinline const int weight();
		__forceinline const uint32_t total();
		__forceinline const bool empty();
	};
	class NavMesh
	{
		Neuron* segments;
	...
	public:
		...
		static __forceinline int DistancePhf(Neuron* a, Neuron* b);
		...
		/// <param name="b">Конечная точка</param>
		/// <returns>Сумма</returns>
		static __forceinline int DistanceManht(Neuron* a, Neuron* b);
