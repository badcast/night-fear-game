...
namespace RoninEngine::Runtime
{
	...
	const float Transform::localAngle() {
		return (this->_parent) ? this->_parent->_angle - this->_angle : this->_angle;
	}
	...
	void Transform::hierarchy_removeAll(Transform* from) {
		Foreach(from->hierarchy, [](Transform* t) {
			t->_parent = NULL;
		});
	...
	void Transform::hierarchy_append(Transform* from, Transform* off) {
		auto iter = find_if(begin(from->hierarchy), end(from->hierarchy), [&off](Transform* of) {
			return of == off;
		});
