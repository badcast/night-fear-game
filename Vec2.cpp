...
const Vec2 Vec2::infinity = Vec2(Infinity, Infinity);
const Vec2 Vec2::negativeInfinity = Vec2(Infinity, Infinity) * -1;
...
float Vec2::SignedAngle(const Vec2& from, const Vec2& to) {
	Vec2 norm = from; norm.Normalize();
	Vec2 norm2 = to; norm2.Normalize();
...
Vec2 RoninEngine::operator *(const Vec2& rhs, const float& d) {
	return d * rhs;
}
